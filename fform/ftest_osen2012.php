...
$res=mysql_query("SELECT * FROM test_osen2012 WHERE ip='$ip'");
...
if (isset($form[email]) && $red == 0){
  ...
  $res=mysql_query("SELECT * FROM test_osen2012 WHERE email='$form[email]'");
  $tmp=mysql_fetch_assoc($res);
  if (count($tmp) > 1){
...
$buf=loginform_show("./fform/_test_osen2012.lf"); //ВНИМАНИЕ: ТУТ НУЖНО УКАЗАТЬ ИМЯ ФОРМЫ

if (isset($onSubmit) && !isset($error)){
         $datereg=time();
	$form[email] = strtolower($form[email]);
  	if ($red == 1){
		$query ="UPDATE test_osen2012";
  	}else{
		$query ="INSERT INTO test_osen2012";
